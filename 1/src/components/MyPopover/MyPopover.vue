<template>
  <div class="custom-box">
    <div class="custom"
         @click="onCustom"
         :style="isCustom?'border: 1px solid rgba(64, 158, 255, 1);':''">
      <i class="el-icon-s-tools"
         style="margin-right:8px;color: #333;font-size: 18px;"></i>
      <span>自定义显示列</span>
    </div>
      ...
      <div class="custom-content"
        ...
        <div class="custom-content-body">
          <el-checkbox style="text-align:left;width: 110px;margin-right: 0"
                       ...
                       :key="index"
                       v-model="custom[index].state">{{item.name}}
          </el-checkbox>
        </div>
        <div class="custom-content-footer">
          ...
          <div style="display:flex;">
            <span class="custom-content-btn custom-content-btn-info"
                  @click.stop="recoverCustom">取消</span>
...
export default {
  ...
  methods: {
    ...
    recoverCustom () {
      this.custom = []
      // this.custom = [...this.backupsCustom]
      this.custom = JSON.parse(JSON.stringify(this.backupsCustom))
      // this.isCustom = false
    },
    closeCustom () {
      this.custom = []
    ...
    handleCustomDefault () {
      ...
      this.defaultFun(this.customList)
    }
  },
  props: {
    customList: {
