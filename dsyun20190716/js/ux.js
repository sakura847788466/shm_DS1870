define(function(require, exports, module) {
	var $ = require('zepto') //他其实和jquery是一样的
		...
		UrlJoint6 = constant.EInvoicePrint_Email_1
	...
	*/
	...
	var constants = {
		...
		eamilPrinter: function() {
			return UrlJoint6 + '/v2.0/mail/emailAccount/'
		}
	}
		...
		var html = (`
				<section class="locker_warp">
				    <section class="locker">
				    	...
				    	<section class="locker_img"><img src="../img/public/error.png"/></section>
...
//		var thr = sessionStorage.getItem("threshold")
	...
	$('.email').on('click',function(){
		...
		$.ajax({
			type: "GET",
			url: constants.eamilPrinter()+printNumber,
			complete: function(xhr) {

				var responseText =  JSON.parse(xhr.responseText);
				var emailAddr = responseText.data;
			
				$('#email-addr').html('');
				var emailAddrContent = '<a href="mailto:'+emailAddr+'?subject=得实云打印">'+emailAddr+'</a> '
				$('#email-addr').append(emailAddrContent);
				$('.locker_blur').css('display','block')
	...
	$('.close-email-box').on('click',function(){
		$('.locker_blur').css('display','none')
