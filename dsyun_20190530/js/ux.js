										...
										$.ajax({
											...
											success: function(res) {
												...
												for(var i = 0; i < res.length; i++) {
													...
													//if(datas!=undefined){
													//	if(res1.user_info.buyer_number == "" && res1.user_info.buyer_bank_account == "") {
													//		lockers('打印失败：限制打印！');
													//		return false;
													//	}
													//	if(res1.user_info.buyer_number == datas.纳税人识别号 || res1.user_info.buyer_bank_account == datas.公司名称) {
													//		
													//	} else {
													//		lockers('打印失败：限制打印！');
													//		return false;
...
if(bba == undefined){
		...
		if(len == 0) {
			...
			if(spac.length == 0) {
				...
				if($(".munber_status").html() != "正常") {
					//    	if($(".munber_status").html() == "设备离线"){
					//    		$(".munber_status").html("离线")
					//    	}
	...
	function PrintService(num) {
		...
		if(types == "file") {
			if(space[num].Url.indexOf("image") != -1) {
				spaceType = "photo"
			}else {
				spaceType = "pdf"
			}
		...
		$.ajax({
			...
			complete: function(xhr) {
				...
				var bs = [],
					ks = [],
					info, html = "";
				//      var ks = JSON.stringify(res.data)
				...
				console.log(bs)
				//alert(info)
				//    info = ks.substring(1, ks.length-1)
				if(info.indexOf("打印成功") != -1) {
					info = '打印成功'
				} else if(info.indexOf("设备离线") != -1) {
					info = '无法打印，设备已离线'
...
//		sessionStorage.setItem("isAdBlock",isAdBlock)
...
//		var isAdBlocks = sessionStorage.getItem("isAdBlock")
		...
		if(credentials == true) {
			if(space[Index].uploadParameter != "file") {
				...
				if(isAdBlocks){
					formData.append('isAdBlock', isAdBlocks)
				}
				
			} else {
				var formData = new FormData()
				
				//var typess = space[Index].name.split(".")[1];
				//if(typess == "pdf"){
				//	var pdfType = "pdf"
				//}else{
				//	var pdfType = "photo"
				//}
				if(space[Index].Url.indexOf("image") != -1) {
					pdfType = "photo"
				}else {
					pdfType = "pdf"
				}
