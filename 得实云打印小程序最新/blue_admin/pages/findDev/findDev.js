...
Page({
    ...
    that.setData({
      ...
      timeId: setTimeout(function () {
        ...
        if (that.data.btDevices.length==1){
          wx.hideLoading();
          wx.navigateTo({
            url: '/pages/cardBag/cardBag?devId=' + that.data.btDevices[0].devId + '&devName=' + encodeURIComponent(that.data.btDevices[0].name),
          })
  ...
  clickDetail:function(option){
    ...
    wx.navigateTo({
      url: '/pages/cardBag/cardBag?devId=' + option.currentTarget.dataset.deviceId + '&devName=' + encodeURIComponent(option.currentTarget.dataset.name),
    })
  }
});
