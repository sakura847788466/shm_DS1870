...
$(function () {
  ...
  } else if (ipBenDi == null) {
    console.log("不存在" + null)
    $('.ipTrue').click(() => {
      if ($('.el-input__inner').val() == '') {
        alert("请输入ip地址")
      } else if (status_IP == true) {
        ...
        //缓存本地localStrong
        window.localStorage.setItem('IP', API)
...
function submitT() {
  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      if (res == 0) {
        alert("提交成功")
        $(".add button").attr("disabled", true).css("background", '#d1d6dc').css("color", '#00000')
        $(".submit button").attr("disabled", true).css("background", '#d1d6dc').css("color", '#00000')
